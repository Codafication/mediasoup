...
class UdpSocketLoopback
{
public:
	/* Struct for the data field of uv_req_t when sending a datagram. */
	struct UvSendData
	{
		UdpSocketLoopback* socket{ nullptr };
		uv_udp_send_t req;
		uint8_t store[1];
	};

public:
	UdpSocketLoopback(const std::string& ip, uint16_t port);
	/**
	 * uvHandle must be an already initialized and binded uv_udp_t pointer.
	 */
	explicit UdpSocketLoopback(uv_udp_t* uvHandle);
	UdpSocketLoopback& operator=(const UdpSocketLoopback&) = delete;
	UdpSocketLoopback(const UdpSocketLoopback&)            = delete;

protected:
	virtual ~UdpSocketLoopback();
...
protected:
	...
	virtual void UserOnUdpSocketLoopbackClosed() = 0;
...
inline void UdpSocketLoopback::Send(const std::string& data, const struct sockaddr* addr)
{
	Send(reinterpret_cast<const uint8_t*>(data.c_str()), data.size(), addr);
}

inline void UdpSocketLoopback::Send(const std::string& data, const std::string& ip, uint16_t port)
{
	Send(reinterpret_cast<const uint8_t*>(data.c_str()), data.size(), ip, port);
}

inline const struct sockaddr* UdpSocketLoopback::GetLocalAddress() const
{
	return reinterpret_cast<const struct sockaddr*>(&this->localAddr);
}

inline int UdpSocketLoopback::GetLocalFamily() const
{
	return reinterpret_cast<const struct sockaddr*>(&this->localAddr)->sa_family;
}

inline const std::string& UdpSocketLoopback::GetLocalIP() const
{
	return this->localIP;
}

inline uint16_t UdpSocketLoopback::GetLocalPort() const
{
	return this->localPort;
}

inline size_t UdpSocketLoopback::GetRecvBytes() const
{
	return this->recvBytes;
}

inline size_t UdpSocketLoopback::GetSentBytes() const
{
