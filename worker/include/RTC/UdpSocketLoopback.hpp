...
namespace RTC
{
	class UdpSocketLoopback : public ::UdpSocketLoopback
	{
	public:
		class Listener
		{
	...
	public:
		UdpSocketLoopback(Listener* listener, int addressFamily);

	private:
		~UdpSocketLoopback() override = default;

		/* Pure virtual methods inherited from ::UdpSocketLoopback. */
	public:
		void UserOnUdpDatagramRecv(const uint8_t* data, size_t len, const struct sockaddr* addr) override;
		void UserOnUdpSocketLoopbackClosed() override;
