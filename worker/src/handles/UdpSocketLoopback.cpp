#define MS_CLASS "UdpSocketLoopback"
// #define MS_LOG_DEV

#include "handles/UdpSocketLoopback.hpp"
...
{
	static_cast<UdpSocketLoopback*>(handle->data)->OnUvRecvAlloc(suggestedSize, buf);
}

inline static void onRecv(
  uv_udp_t* handle, ssize_t nread, const uv_buf_t* buf, const struct sockaddr* addr, unsigned int flags)
{
	static_cast<UdpSocketLoopback*>(handle->data)->OnUvRecv(nread, buf, addr, flags);
}

inline static void onSend(uv_udp_send_t* req, int status)
{
	auto* sendData            = static_cast<UdpSocketLoopback::UvSendData*>(req->data);
	UdpSocketLoopback* socket = sendData->socket;

	// Delete the UvSendData struct (which includes the uv_req_t and the store char[]).
	std::free(sendData);

	// Just notify the UdpSocketLoopback when error.
	if (status != 0)
...
{
	static_cast<UdpSocketLoopback*>(handle->data)->OnUvClosed();
}
...
UdpSocketLoopback::UdpSocketLoopback(const std::string& ip, uint16_t port)
{
	MS_TRACE();
...
UdpSocketLoopback::UdpSocketLoopback(uv_udp_t* uvHandle) : uvHandle(uvHandle)
{
	MS_TRACE();
...
UdpSocketLoopback::~UdpSocketLoopback()
{
	MS_TRACE();
...
void UdpSocketLoopback::Destroy()
{
	MS_TRACE();
...
void UdpSocketLoopback::Dump() const
{
	MS_DUMP("<UdpSocketLoopback>");
	MS_DUMP(
...
void UdpSocketLoopback::Send(const uint8_t* data, size_t len, const struct sockaddr* addr)
{
	MS_TRACE();
...
void UdpSocketLoopback::Send(const uint8_t* data, size_t len, const std::string& ip, uint16_t port)
{
	MS_TRACE();
...
bool UdpSocketLoopback::SetLocalAddress()
{
	MS_TRACE();
...
inline void UdpSocketLoopback::OnUvRecvAlloc(size_t /*suggestedSize*/, uv_buf_t* buf)
{
	MS_TRACE();
...
inline void UdpSocketLoopback::OnUvRecv(
...
inline void UdpSocketLoopback::OnUvSendError(int /*error*/)
{
	MS_TRACE();
...
inline void UdpSocketLoopback::OnUvClosed()
{
	MS_TRACE();

	// Notify the subclass.
	UserOnUdpSocketLoopbackClosed();
